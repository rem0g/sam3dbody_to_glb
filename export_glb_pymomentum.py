...
# ── Hand parameter decoding (NumPy port of mhr_utils.py) ──
...
# Precompute boolean masks (same logic as mhr_utils.py)
...
# Model parameter indices that control leg joint rotations/movements
# (identified via parameter_transform analysis - only affects leg joints)
_LEG_ROTATION_INDICES = [
...
def _cont_to_hand_euler(hand_cont):
    """Convert 54-dim continuous hand representation to 27-dim Euler model params."""
    assert hand_cont.shape[-1] == 54

    # 3-DOF joints: 6D continuous -> XYZ euler
    cont_3dof = hand_cont[_MASK_CONT_3DOF].reshape(-1, 6)
    euler_3dof = _xyz_from_6d(cont_3dof).flatten()

    # 1-DOF/2-DOF joints: sin/cos pairs -> atan2
...
def load_head_buffers(assets_dir):
    """Load MHRHead buffers (scale PCA, hand PCA, hand joint indices)."""
    ...
    if not os.path.exists(buf_path):
        raise FileNotFoundError(
            ...
            "Generate with: python -c \"import torch, numpy as np; "
            "sd = torch.load('sam-3d-body/checkpoints/.../model.ckpt', "
            "map_location='cpu', weights_only=False)['state_dict']; "
            "np.savez('MHR/assets/mhr_head_buffers.npz', "
            "scale_mean=sd['head_pose.scale_mean'].numpy(), ...)\""
        )
...
def build_model_params(data, head_buffers, every=1):
    """
    Correctly assemble (N, 204) model params from sam3dbody fields.

    Replicates MHRHead.mhr_forward() parameter assembly:
      [global_trans*10 (3), global_rot (3), body_pose[:130] (130)] = 136
      + hand euler angles inserted at specific indices
      + scales from PCA (68)
      = 204
    """
    body_pose = data["body_pose_params"]      # (N, 133) euler model params
    hand_pose = data["hand_pose_params"]      # (N, 108) PCA coefficients
    global_rot = data["global_rot"]           # (N, 3) euler angles

    num_frames = body_pose.shape[0]

    # Scale params (may be missing from older sam3dbody files)
    if "scale_params" in data:
        scale_params = data["scale_params"]   # (N, 28) PCA coefficients
    else:
    ...
    for i in frame_indices:
        # 1. Assemble full_pose_params: [global_trans*10, global_rot, body_pose[:130]]
        #    global_trans is always zeros (not stored in sam3dbody)
        full_pose = np.zeros(136, dtype=np.float64)
        full_pose[3:6] = global_rot[i]
        full_pose[6:136] = body_pose[i, :130]

        # 2. Decode hand PCA coefficients -> continuous -> euler -> insert
        ...
        # 3. Compute scales via PCA
        scales = scale_mean + scale_params[i] @ scale_comps  # (68,)

        # 4. Final model_params
        mp = np.concatenate([full_pose, scales])  # (204,)
...
def freeze_legs(model_params):
    """Zero out leg rotation parameters to freeze legs in rest pose."""
    model_params = model_params.copy()
    model_params[:, _LEG_ROTATION_INDICES] = 0.0
    return model_params


def freeze_root(model_params):
    """Zero out global rotation to keep body upright and centered."""
    model_params = model_params.copy()
    # Indices 3, 4, 5 = root_rx, root_ry, root_rz (global body rotation)
    model_params[:, 3:6] = 0.0
    return model_params


def smooth_params(model_params, sigma):
    """Apply Gaussian temporal smoothing to model parameters."""
    from scipy.ndimage import gaussian_filter1d
    # Smooth each parameter independently along the time axis
